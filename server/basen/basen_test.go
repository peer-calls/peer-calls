...
func TestEncodeDecode_base16(t *testing.T) {
	encoder := basen.NewEncoder(basen.AlphabetBase16)
	decoder := basen.NewDecoder(basen.AlphabetBase16)
...
func TestEncodeDecode_base62(t *testing.T) {
	encoder := basen.NewEncoder(basen.AlphabetBase62)
	decoder := basen.NewDecoder(basen.AlphabetBase62)
	for _, data := range getCases(0x1, 0xFFFF) {
		result := encoder.Encode(data)
		data2, err := decoder.Decode(result)
		assert.Nil(t, err)
...
func TestEncodeDecode_base64(t *testing.T) {
	encoder := basen.NewEncoder(basen.AlphabetBase64)
	decoder := basen.NewDecoder(basen.AlphabetBase64)
	for _, data := range getCases(0x1, 0xFFFF) {
		result := encoder.Encode(data)
		data2, err := decoder.Decode(result)
		assert.Nil(t, err)
