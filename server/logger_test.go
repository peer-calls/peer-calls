...
func TestGetLogger_Printf(t *testing.T) {
	...
	serverFactory := server.NewLoggerWriterFactoryFromEnv("TESTLOG_", &out)
	logB := serverFactory.GetLogger("b")
	logC := serverFactory.GetLogger("c")
...
func TestSetDefaultEnabled(t *testing.T) {
	...
	var out strings.Builder
	serverFactory := server.NewLoggerWriterFactoryFromEnv("TESTLOG_", &out)
	serverFactory.SetDefaultEnabled([]string{"b"})
	logB := serverFactory.GetLogger("b")
	logB.Println(1, "one")
	assert.Regexp(t, " \\[              b] 1 one\n", out.String())
}
...
func TestGetLogger_Wildcard_Middle(t *testing.T) {
	...
	var out strings.Builder
	serverFactory := server.NewLoggerWriterFactoryFromEnv("TESTLOG_", &out)
	logAOneWarn := serverFactory.GetLogger("a:one:warn")
	logAOneInfo := serverFactory.GetLogger("a:one:info")
	logATwoWarn := serverFactory.GetLogger("a:two:warn")
	logATwoInfo := serverFactory.GetLogger("a:two:info")
	logBOneWarn := serverFactory.GetLogger("b:one:warn")
